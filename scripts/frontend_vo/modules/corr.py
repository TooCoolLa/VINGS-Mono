...
'''
Maybe our CorrSampler's droid_backends_nerfslam is different???
'''
...
class CorrSampler(torch.autograd.Function):
    ...
    def forward(ctx, volume, coords, radius):
        ...
        ctx.radius = radius
        corr, = droid_backends_nerfslam.corr_index_forward(volume, coords, radius)
        return corr
    ...
    def backward(ctx, grad_output):
        ...
        grad_volume, = droid_backends_nerfslam.corr_index_backward(volume, coords, grad_output, ctx.radius)
...
class CorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=3):
        ...
        self.corr_pyramid = []
        '''
        fmap1.shape = torch.Size([1, 1, 128, 85, 150])
        corr.shape = torch.Size([12750, 1, 85, 150])
        '''
...
class CorrLayer(torch.autograd.Function):
    ...
    def forward(ctx, fmap1, fmap2, coords, r):
        ...
        corr, = droid_backends_nerfslam.altcorr_forward(fmap1, fmap2, coords, ctx.r)
        return corr
    ...
    def backward(ctx, grad_corr):
        ...
        fmap1_grad, fmap2_grad, coords_grad = \
            droid_backends_nerfslam.altcorr_backward(fmap1, fmap2, coords, grad_corr, ctx.r)
