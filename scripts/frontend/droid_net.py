...
from frontend.geom.ba import BA
import frontend.geom.projective_ops as pops
from frontend.geom.graph_utils import graph_to_edge_list, keyframe_indicies
...
def cvx_upsample(data, mask):
    ...
    '''
    data.shape = [8, 43, 77, 1]
    mask.shape = [1, 8, 576, 43, 77]
    '''
    batch, ht, wd, dim = data.shape
    data = data.permute(0, 3, 1, 2) # [8, 1, 43, 77]
    mask = mask.view(batch, 1, 9, 8, 8, ht, wd) # [8, 1, 9, 8, 8, 43, 77]
    mask = torch.softmax(mask, dim=2)           # [8, 1, 9, 8, 8, 43, 77]

    up_data = F.unfold(data, [3,3], padding=1)          # [8, 9, 3311]
    
    up_data_1 = up_data.view(batch, dim, 9, 1, 1, ht, wd) # updata_1, [8, 1, 9, 1, 1, 43, 77]

    up_data = torch.sum(mask * up_data_1, dim=2) # updata_2, [8, 1, 8, 8, 43, 77]
