...
class KintinuousDataset:
    ...
    def preload_rgbinfo(self):
        ...
        '''
        rgb_files = sorted(glob.glob(os.path.join(self.dataset_dir, 'nosky_color', '*.png')))
...
class Fake_Calib:
    def __init__(self, Dataset):
        ...
        fv, fu, cv, cu = Dataset.cfg['intrinsic']['fv'], Dataset.cfg['intrinsic']['fu'], Dataset.cfg['intrinsic']['cv'], Dataset.cfg['intrinsic']['cu']
        fv, cv = Dataset.w1/Dataset.cfg['intrinsic']['W']*fv, Dataset.w1/Dataset.cfg['intrinsic']['W']*cv
        fu, cu = Dataset.h1/Dataset.cfg['intrinsic']['H']*fu, Dataset.h1/Dataset.cfg['intrinsic']['H']*cu
        self.camera_model = torch.tensor([fv, fu, cv, cu])
        

def get_dataset(config):
    return KintinuousDataset(config)