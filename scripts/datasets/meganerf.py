...
class MegaNeRFDataset:
    ...
    def preload_rgbinfo(self):
        ...
        '''
        rgb_files = sorted(glob.glob(os.path.join(self.dataset_dir, 'rgbs_4', '*.jpg')))
    ...
    def load_gt_dict(self):
        c2w_files    = os.listdir(os.path.join(self.dataset_dir, 'poses'))
        c2ws         = np.array([np.loadtxt(os.path.join(self.dataset_dir, 'poses', c2w_file)) for c2w_file in c2w_files])
        timestamps   = np.array(list(map(lambda x: float(x.split('.')[0].strip('DJI_')), c2w_files)))
...
RGB4_DATA_DIR = '/data/wuke/DATA/2024/MegaNeRF/building-pixsfm/train/rgbs_4/'
META_DATA_DIR = '/data/wuke/DATA/2024/MegaNeRF/building-pixsfm/train/metadata/'
SAVE_DIR      = '/data/wuke/DATA/2024/MegaNeRF/building-pixsfm/train/poses/'


for filename in os.listdir(RGB4_DATA_DIR):
    metadata = torch.load(os.path.join(META_DATA_DIR, filename.replace('.jpg', '.pt')))
    c2w = metadata['c2w'].numpy()
    np.savetxt(os.path.join(SAVE_DIR, filename.replace('.jpg', '.txt')), c2w)
'''
