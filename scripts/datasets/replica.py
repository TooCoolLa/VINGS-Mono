...
class ReplicaDataset:
    def __init__(self, cfg):
        ...
        # Load data/datainfo.
        self.dataset_dir = os.path.join(cfg['dataset']['root'], 'results')
    ...
    def preload_rgbinfo(self):
        ...
        '''
        rgb_files = sorted(glob.glob(os.path.join(self.dataset_dir, '*.jpg')))
    ...
    def load_gt_dict(self):
        c2w_files    = os.listdir(os.path.join(self.cfg['dataset']['root'], 'pose'))
        c2ws         = np.array([np.loadtxt(os.path.join(self.cfg['dataset']['root'], 'pose', c2w_file)) for c2w_file in c2w_files])
