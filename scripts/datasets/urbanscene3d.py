...
class UrbanScene3DDataset:
    ...
    def preload_rgbinfo(self):
        ...
        '''
        rgb_files = sorted(glob.glob(os.path.join(self.dataset_dir, 'rgb_downsample', '*.JPG')), key=lambda x: int(x.split('/')[-1].split('.')[0].strip('DJI_')))
    ...
    def load_gt_dict(self):
        c2w_files    = os.listdir(os.path.join(self.dataset_dir, 'c2w'))
        c2ws         = np.array([np.loadtxt(os.path.join(self.dataset_dir, 'c2w', c2w_file)) for c2w_file in c2w_files])
...
def get_dataset(config):
    return UrbanScene3DDataset(config)