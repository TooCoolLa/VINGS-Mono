...
class PhoneVIODataset:
    ...
    def preload_rgbinfo(self):
        ...
        depth_files = sorted(glob.glob(os.path.join(self.dataset_dir, 'depth', '*.npy')))
        self.depthinfo_dict = {}
        self.depthinfo_dict['timestamp'] = list(range(len(depth_files)))    # (N, ), list
        self.depthinfo_dict['filepath']  = depth_files # (N, ), list
