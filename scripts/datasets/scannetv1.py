...
class ScanNetV1Dataset:
    ...
    def preload_rgbinfo(self):
        ...
        '''
        rgb_files = sorted(glob.glob(os.path.join(self.dataset_dir, 'color', '*.jpg')), key=lambda x: int(x.split('/')[-1].split('.')[0]))
...
def get_dataset(config):
    return ScanNetV1Dataset(config)